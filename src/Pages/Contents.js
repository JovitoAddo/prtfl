import React from "react";
import funky from "../Assets/funky.png";
import weirdo from "../Assets/weirdo.jpg";
function Contents() {
  return (
    <div className="flex flex-col content-center " style={{}}>
      <div className=" ">
        <div className="flex justify-center pt-3  ">
          <img
            src={weirdo}
            className="rounded-full border-double border-8 border-slate-700"
            style={{ height: "250px" }}
          ></img>
        </div>
        <div className="container mx-auto text-white grid pt-4  ">
          <p>
            what am i suppose to write here? Dolor non velit voluptate ad amet
            ut in cillum ad eiusmod sit proident ullamco fugiat. Laborum nostrud
            elit cillum fugiat amet commodo ullamco officia dolor cupidatat.
            Sunt Lorem ex tempor laborum incididunt. Ad reprehenderit esse
            excepteur commodo ex qui id et Lorem. Et aliquip duis do ex
            voluptate qui proident laboris laboris minim officia voluptate ut.
            Minim labore ullamco aute eiusmod. Consequat ut et ipsum nostrud
            dolor dolor qui id.
          </p>
        </div>
      </div>
      <svg
        width="100%"
        height="100%"
        id="svg"
        viewBox="0 0 1440 400"
        xmlns="http://www.w3.org/2000/svg"
        class="transition duration-300 ease-in-out delay-150"
      >
        <path
          d="M 0,400 C 0,400 0,200 0,200 C 44.13920551637186,205.2657197650513 88.27841103274372,210.53143953010257 134,194 C 179.72158896725628,177.46856046989743 227.02556138539694,139.13996164464095 276,126 C 324.97443861460306,112.86003835535905 375.6193434256684,124.90871389133358 415,128 C 454.3806565743316,131.09128610866642 482.49706491192967,125.2251827900248 522,133 C 561.5029350880703,140.7748172099752 612.3923969266131,162.19055494856735 662,170 C 711.6076030733869,177.80944505143265 759.9333473816182,172.01259741570584 799,186 C 838.0666526183818,199.98740258429416 867.8742135469145,233.7590553886092 902,234 C 936.1257864530855,234.2409446113908 974.569798430724,200.95118102985734 1030,171 C 1085.430201569276,141.04881897014266 1157.84659273019,114.43622049196148 1201,126 C 1244.15340726981,137.56377950803852 1258.043830648517,187.30393700229672 1293,206 C 1327.956169351483,224.69606299770328 1383.9780846757415,212.34803149885164 1440,200 C 1440,200 1440,400 1440,400 Z"
          stroke="none"
          stroke-width="0"
          fill="#4b5870ff"
          class="transition-all duration-300 ease-in-out delay-150 path-0"
        ></path>
      </svg>
      <p style={{ backgroundColor: "#4b5870" }}>
        Magna velit occaecat ullamco minim magna est nisi aliquip. Cillum aliqua
        do dolore et velit deserunt culpa nisi mollit. Amet sunt laborum est non
        reprehenderit fugiat adipisicing duis mollit enim anim sint qui. Sunt
        reprehenderit ea Lorem tempor sunt enim do officia excepteur duis id ad
        eiusmod adipisicing. Ex ullamco ea amet sit eu in est quis consectetur
        irure ex elit quis pariatur.Enim ut aute cupidatat dolore ad commodo
        deserunt commodo nisi mollit ullamco et. Dolore incididunt quis
        consectetur tempor culpa consequat consectetur cupidatat ullamco culpa
        labore est esse Lorem. Pariatur irure magna fugiat nostrud laboris. Ut
        in sit dolore pariatur labore dolor qui.
      </p>
    </div>
  );
}
export default Contents;
